...
public  interface LEvents {
  ...
  /** :: DeveloperApi ::
   * Insert {@link Event}s in a non-blocking fashion.
   * <p>
   * Default implementation of this method is calling
   * {@link LEvents.futureInsert(Event, Int, Option[Int])} per event.
   * Override in the storage implementation if the storage has
   * a better way to insert multiple data at once.
   * <p>
   * @param events {@link Event}s to be inserted
   * @param appId App ID for the {@link Event}s to be inserted to
   * @param channelId Optional channel ID for the {@link Event}s to be inserted to
   * @param ec (undocumented)
   * @return (undocumented)
   */
  public  scala.concurrent.Future<scala.collection.Seq<java.lang.String>> futureInsertBatch (scala.collection.Seq<org.apache.predictionio.data.storage.Event> events, int appId, scala.Option<java.lang.Object> channelId, scala.concurrent.ExecutionContext ec)  ;
  /** :: DeveloperApi ::
  ...
  /**
   * @deprecated Use find() instead. Since 0.9.2. 
  */
   ...
   * @param timeout (undocumented)
   *
   * @deprecated Use LEventStore.findByEntity() instead. Since 0.9.2. 
  */
